...
if __name__ == "__main__":
    ...
    for parallel in ["AR-8", "AR-16", "AR-32", "DNQ", "Pipeline"]:
        for dataset in ["AustraliaTourism"]:
            for level in ["C", "M", "F"]:
                ...
                if level == "C":
                    print(f" {mapper[parallel]} & ${val:.3f} \\pm \\text{{\\scriptsize{{{std:.3f}}}}}$ & ${int(queries)}$", end="")
                elif level == "M":
                    print(f" & ${val:.3f} \\pm \\text{{\\scriptsize{{{std:.3f}}}}}$ & ${int(queries)}$", end="")
                else:
                    print(f" & ${val:.3f} \\pm \\text{{\\scriptsize{{{std:.3f}}}}}$ & ${int(queries)}$\\\\")
